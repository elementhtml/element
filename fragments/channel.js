const initializeSSEChannel = async function () {
    ...
    this.channel.addEventListener('error', () => reopenSSEChannel())
}, reopenSSEChannel = async function () {
    (typeof requestIdleCallback === 'function') ? requestIdleCallback(() => initializeSSEChannel(config)) : setTimeout(() => initializeSSEChannel(config), 1000)
}, initializeWebSocket = async function (config) {
    this.channel = new WebSocket(config.url, config.protocols ?? [])
    this.channel.addEventListener('message', event => this.eventTarget.dispatchEvent(new CustomEvent('message', { detail: event.data })))
    this.channel.addEventListener('close', () => reopenWebSocket(config))
    this.channel.addEventListener('error', () => reopenWebSocket(config))
}, reopenWebSocket = async function (config) {
    (typeof requestIdleCallback === 'function') ? requestIdleCallback(() => initializeWebSocket(config)) : setTimeout(() => initializeWebSocket(config), 1000)
    ...
    (typeof requestIdleCallback === 'function') ? requestIdleCallback(() => initializeWebTransportChannel(config)) : setTimeout(() => initializeWebTransportChannel(config), 1000)
...
export default {
    send: async function (message) {
        switch (this.type) {
            ...
            case 'sse': break
            case 'websocket':
                if (this.channel.readyState !== 1) await waitForWebSocketReady()
