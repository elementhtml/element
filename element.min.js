const ElementHTML = Object.defineProperties({}, {
    version: { enumerable: !0, value: '2.0.0' }, env: {
        enumerable: !0, value: {
            ais: {}, apis: {}, components: {}, content: {}, context: {}, facets: {}, gateways: {}, hooks: {}, interpreters: new Map([[/^[#?/:]$/, {
                name: 'router', handler: async function (container, position, envelope, value) {
            ...
            }], [/^(true|false|null|[.!-]|"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|-?\d+(\.\d+)?)$/, { name: 'value', handler: async function (container, position, envelope, value) { return envelope.descriptor.value } }], [/^\$\{.*\}$/, { name: 'variable', handler: async function (container, position, envelope, value) { return this.resolveVariable(descriptor.expression, Object.freeze({ ...envelope, value })) } }], [/^[{](.*?)[}]$|^[\[](.*?)[\]]$|^\?[^ ]+$/, {
                name: 'shape', handler: async function (container, position, envelope, value) {
                    const { descriptor } = envelope
                    return this.resolveVariable(descriptor.shape, Object.freeze({ ...envelope, value }))
                }
            }], [/^#\`[^`]+(\|[^`]+)?\`$/, {
                name: 'content', handler: async function (container, position, envelope, value) {
                    const { descriptor, variables } = envelope, { collection: a, article: articleSignature, lang: langSignature } = descriptor, wrapped = variables && !0, valueEnvelope = variables && Object.freeze({ ...envelope, value }), collection = await this.resolveUnit(variables?.collection ? this.resolveVariable(a, valueEnvelope, { wrapped }) : a, 'collection')
                ...
                name: 'transform', handler: async function (container, position, envelope, value) {
                    let { descriptor, variables } = envelope, { transform: t } = descriptor, vTransform = variables?.transform, wrapped = vTransform && !0, valueEnvelope = vTransform ? Object.freeze({ ...envelope, value }) : envelope, transform = await this.resolveUnit(vTransform ? this.resolveVariable(t, valueEnvelope, { wrapped }) : t, 'transform')
                ...
                name: 'pattern', handler: async function (container, position, envelope, value) {
                    const { descriptor, variables } = envelope, { pattern: p } = descriptor, wrapped = variables && !0, valueEnvelope = variables ? Object.freeze({ ...envelope, value }) : undefined, pattern = await this.resolveUnit(variables.pattern ? this.resolveVariable(p, valueEnvelope, { wrapped }) : p, 'pattern')
                ...
                name: 'type', handler: async function (container, position, envelope, value) {
                    const { descriptor } = envelope, { types, mode } = descriptor, info = mode === 'info', promises = [], wrapped = !0, valueEnvelope = { ...envelope, value }
                    ...
                    if (info) {
                        ...
                        for (const { name } of types) promises.push(this.runUnit(name, 'type', value, !0).then(r => validation[name] = r))
                        await Promise.all(promises)
                ...
                name: 'selector', handler: async function (container, position, envelope, value) {
                    ...
                    const { descriptor } = envelope, { signal } = descriptor, { scope: scopeStatement, selector: selectorStatement } = descriptor, scope = await this.resolveScope(scopeStatement, container), { sys } = this, { defaultEventTypes } = sys
                    if (!scope) return {}
                    const bangIndex = selectorStatement.lastIndexOf('!')
                    let selector = selectorStatement.trim(), eventList
                    if ((bangIndex > selector.lastIndexOf(']')) && (bangIndex > selector.lastIndexOf(')')) && (bangIndex > selector.lastIndexOf('"')) && (bangIndex > selector.lastIndexOf("'"))) [selector, eventList] = [selector.slice(0, bangIndex).trim(), selector.slice(bangIndex + 1).trim()]
                    ...
                    for (let eventName of eventList ?? Array.from(new Set(Object.values(defaultEventTypes).concat(['click'])))) {
                        ...
                        scope.addEventListener(eventName, event => { this.runFragment('env/interpreters/selector', event, selector, scope, exactMatch, defaultEventTypes, keepDefault, container, position) }, { signal, once })
                    }
                    return { selector, scope }
                }
            }], [/^[#@](?:[a-zA-Z0-9]+|[{][a-zA-Z0-9#@?!, ]*[}]|[\[][a-zA-Z0-9#@?!, ]*[\]])$/, {
                name: 'state', handler: async function (container, position, envelope, value) {
                    ...
                    switch (shape) {
                        case 'single': const { type, name } = target
                            target[type] = type === 'field' ? (new this.Field(container, name)) : (new this.Cell(name))
                            getReturnValue = () => target[type].get()
                            items.push(target)
                            break
                        case 'array': for (const t of target) (items[items.length] = t)[t.type] = t.type === 'field' ? (new this.Field(container, t.name)) : (new this.Cell(t.name))
                            getReturnValue = (r = [], l) => {
                                for (const t of target) if ((r[l ??= r.length] = t[t.type].get()) === undefined) return
                                return r
                            }
                            break
                        case 'object': if (Array.isArray(target)) target = Object.fromEntries(target)
                ...
                name: 'api', handler: async function (container, position, envelope, value) {
                    const { descriptor, variables } = envelope, { api: a, action: actionSignature } = descriptor, wrapped = variables && !0, valueEnvelope = Object.freeze({ ...envelope, value }), api = await this.resolveUnit(variables?.api ? this.resolveVariable(a, valueEnvelope, { wrapped }) : a, 'api')
                    if (!api) return
                ...
                name: 'ai', handler: async function (container, position, envelope, value) {
                    const { descriptor, variables } = envelope, { ai: m, prompt: p } = descriptor, wrapped = variables && !0, valueEnvelope = Object.freeze({ ...envelope, value }), ai = await this.resolveUnit(variables?.ai ? this.resolveVariable(m, valueEnvelope, { wrapped }) : a, 'ai')
                    if (!ai) return
                ...
                name: 'request', handler: async function (container, position, envelope, value) { return await this.runFragment('env/interpreters/request', envelope, value) }, binder: async function (container, position, envelope) {
            ...
            }], [/^_.*_$/, { name: 'wait', handler: async function (container, position, envelope, value) { return await this.runFragment('env/interpreters/wait', container, position, envelope, value) } }], [/^\$\`[^`]+\`$/, { name: 'command', handler: async function (container, position, envelope, value) { return this.modules.dev ? (await this.runFragment('env/interpreters/command', envelope, value)) : value } }], [/^\$\??$/, { name: 'console', handler: async function (container, position, envelope, value) { return this.modules.dev ? ((envelope.descriptor.verbose === !0 ? (console.log(await this.flatten({ container, position, envelope, value }))) : (console.log(value))) ?? value) : value } }]]), languages: {}, libraries: { jsonata: 'https://cdn.jsdelivr.net/npm/jsonata@2.0.5/+esm', md: 'https://cdn.jsdelivr.net/npm/remarkable@2.0.1/+esm#Remarkable', schema: 'https://cdn.jsdelivr.net/gh/nuxodin/jema.js@1.2.0/schema.min.js#Schema', xdr: 'https://cdn.jsdelivr.net/gh/cloudouble/simple-xdr/xdr.min.js' }, models: {}, namespaces: { e: new URL(`./components`, import.meta.url) }, patterns: {}, renderers: {}, resolvers: {}, snippets: {}, transforms: { 'application/json': (E) => (new E.Transform((input) => { try { return JSON.stringify(input) } catch (e) { } })), 'text/markdown': import.meta.resolve('./transforms/md.js'), 'application/x-xdr': `${import.meta.resolve('./transforms/xdr.js')}#application`, 'text/x-xdr': `${import.meta.resolve('./transforms/xdr.js')}#text`, }, types: {}
        }
    }, Compile: { enumerable: !0, value: function () { return this.runFragment('compile') } }, Dev: { enumerable: !0, value: function () { return this.runFragment('dev') } }, Expose: { enumerable: !0, value: function (name) { window[name || 'E'] ??= this } }, ImportPackage: {
        enumerable: !0, value: async function (pkg, packageUrl, packageKey) {
            ...
            for (const unitTypeCollectionName in pkg) if (unitTypeCollectionName in env) {
                let unitTypeCollection = (typeof pkg[unitTypeCollectionName] === 'string') ? this.resolveImport(this.resolveUrl(pkg[unitTypeCollectionName], packageUrl), !0) : Promise.resolve(pkg[unitTypeCollectionName])
        ...
        enumerable: !0, value: async function (rootElement = undefined) {
            ...
            env.interpreters = Object.freeze(new Proxy(interpreters, { set: interpretersProxyError, delete: interpretersProxyError, clear: interpretersProxyError, get: (target, prop) => (typeof target[prop] === 'function') ? target[prop].bind(target) : Reflect.get(target, prop) }))
            Object.freeze(env)
            Object.freeze(app)
    ...
    }, createEnvelope: { enumerable: !0, value: async function (value = {}) { return Object.freeze({ ...(this.isPlainObject(value) ? value : { value }), cells: await this.flatten(this.app.cells), context: this.env.context }) } }, deepFreeze: {
        enumerable: !0, value: function (obj, copy, isArray) {
        ...
        enumerable: !0, value: async function (value, event) {
            ...
            switch (value?.constructor) {
                ...
                case DataTransfer: return Object.defineProperties({ dropEffect: value.dropEffect, effectAllowed: value.effectAllowed, types: value.types }, { files: { enumerable: !0, get: () => this.flatten(value.files) }, items: { enumerable: !0, get: () => this.flatten(value.items) } })
                case FileList: case DataTransferItemList: case Array: let a = []
                    for (const f of value) a.push(this.flatten(f))
                    return Promise.all(a)
                case FormData: return Object.fromEntries(value.entries())
                case Response: return Object.defineProperties({ ok: value.ok, redirected: value.redirected, status: value.status, statusText: value.statusText, type: value.type, url: value.url }, { body: { enumerable: !0, get: () => this.parse(value) }, bodyUsed: { enumerable: !0, get: () => value.bodyUsed }, headers: { enumerable: !0, get: () => Object.fromEntries(value.headers.entries()) } })
                default: if (typeof value.valueOf === 'function') return value.valueOf()
            ...
            if (value instanceof HTMLElement) {
                ...
                return new Proxy({}, { get: (target, prop) => processElementMapper.call(this, value, prop, mappers), has: (target, prop) => processElementMapper.call(value, prop, mappers, !0) })
            }
            for (const p in this) if ((p.charCodeAt(0) <= 90) && (this[p].prototype instanceof this[p]) && value instanceof this[p]) return value.valueOf()
        }
    }, generateUuid: { enumerable: !0, value: function (noDashes) { return crypto?.randomUUID()?.[noDashes ? 'replace' : 'toString'](this.sys.regexp.dash, '') ?? (noDashes ? 'xxxxxxxxxxxx4xxxyxxxxxxxxxxxxxxx' : 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx').replace(this.sys.regexp.xy, c => ((c === 'x' ? Math.random() * 16 : (Math.random() * 4 + 8)) | 0).toString(16)) } }, getCustomTag: {
        enumerable: !0, value: function (element) {
    ...
    }, isFacetContainer: { enumerable: !0, value: function (element) { return ((element instanceof HTMLScriptElement) && element.type?.endsWith('/element')) } }, isPlainObject: {
        enumerable: !0, value: function (obj) {
            if (!obj) return !1
            const proto = Object.getPrototypeOf(obj)
            return (proto === null) || (proto === Object.prototype) || (proto.constructor === Object)
        }
    }, isWrappedVariable: { enumerable: !0, value: function (expression) { return ((expression[0] === '$') && (expression[1] === '{') && (expression.endsWith('}'))) } }, parse: { enumerable: !0, value: async function (input, contentType) { return this.runFragment('parse', input, contentType) } }, render: { enumerable: !0, value: async function (element, data) { return this.runFragment('render', element, data) } }, resolveImport: {
        enumerable: !0, value: async function (importHref, returnWholeModule, isWasm) {
            const { hash = '#default', origin, pathname } = (importHref instanceof URL) ? importHref : this.resolveUrl(importHref, undefined, !0), url = `${origin}${pathname}`, module = (isWasm ?? pathname.endsWith('.wasm')) ? (await WebAssembly.instantiateStreaming(fetch(url))).instance.exports : await import(url)
            return returnWholeModule ? module : module[hash.slice(1)]
        }
    }, resolveScope: {
        enumerable: !0, value: async function (scopeStatement, element) {
            ...
            switch (scopeStatement) {
                ...
                case '*': case '&': case '@': let scope = element.getRootNode()
                    switch (scopeStatement) {
        ...
        enumerable: !0, value: async function (scopedSelector, element) {
        ...
        enumerable: !0, value: async function (selector, scope) {
            ...
            try { return isMulti ? this.sliceAndStep(sliceSignature, Array.from(scope.querySelectorAll(selector))) : scope.querySelector(selector) } catch (e) { return this.runFragment('resolveselector', selector, scope, isMulti, sliceSignature) }
        }
    }, resolveUnit: {
        enumerable: !0, value: async function (unitKey, unitType) {
        ...
        enumerable: !0, value: function (value, base, raw) {
            ...
            if (gateway) {
                ...
                switch (typeof gateway) {
                    case 'function': const gatewayArgs = { path }
                    ...
                    case 'string': const mergedUrl = new URL(gateway.replace(regexp.gatewayUrlTemplateMergeField, (match, mergeExpression) => {
        ...
        enumerable: !0, value: function (expression, envelope = {}, flags = {}) {
            expression = expression.trim()
            const { sys, resolveVariable } = this, { regexp, valueAliases } = sys
            let result, { wrapped = !1, default: dft = (!wrapped ? expression : undefined), spread, merge } = flags
            if (merge) { result = expression.replace(regexp.hasVariable, (match, varExpression) => (resolveVariable.call(this, varExpression, envelope) ?? match)) } else if (typeof expression === 'string') {
                ...
                if (expression in valueAliases) result = valueAliases[expression]
                    ...
                    if (context || cells || fields || labels || ('value' in envelope)) result = resolveVariable.call(this, expression, envelope, { ...flags, wrapped: !1 })
        ...
        enumerable: !0, value: async function (unitKey, unitType, ...args) {
            const unit = await this.resolveUnit(unitKey, unitType), isArray = Array.isArray(unit), isObject = !isArray && this.isPlainObject(unit), promises = (isArray || isObject) && [], result = isObject && {}
            if (!unit) return
            if (isArray) {
    ...
    }, serialize: { enumerable: !0, value: async function (input, contentType = 'application/json') { return this.runFragment('serialize', input, contentType) } }, toCamelCase: {
        enumerable: !0, value: function (str) {
    ...
    }, toKebabCase: { enumerable: !0, value: function (str) { return str.replace(this.sys.regexp.lowerCaseThenUpper, '$1-$2').replace(this.sys.regexp.upperCaseThenAlpha, '$1-$2').toLowerCase() } }, app: { value: Object.defineProperties({}, { cells: { enumerable: !0, value: {} }, _components: { value: { nativesFromVirtuals: new WeakMap(), bindings: new WeakMap(), virtualsFromNatives: new WeakMap() } }, _eventTarget: { value: new EventTarget() }, _facetInstances: { value: new WeakMap() }, _fragments: { value: {} }, _observers: { value: new WeakMap() } }) }, modules: { enumerable: !0, value: {} }, sys: { value: Object.freeze({ defaultEventTypes: Object.freeze({ audio: 'loadeddata', body: 'load', details: 'toggle', dialog: 'close', embed: 'load', form: 'submit', iframe: 'load', img: 'load', input: 'change', link: 'load', meta: 'change', object: 'load', script: 'load', search: 'change', select: 'change', slot: 'slotchange', style: 'load', textarea: 'change', track: 'load', video: 'loadeddata' }), impliedScopes: Object.freeze({ ':': '*', '#': 'html' }), localOnlyUnitTypes: new Set(['hook']), locationKeyMap: { '#': 'hash', '/': 'pathname', '?': 'search' }, queue: new Map(), regexp: Object.freeze({ commaSplitter: /\s*,\s*/, colonSplitter: /\s*\:\s*/, dashUnderscoreSpace: /[-_\s]+(.)?/g, extractAttributes: /(?<=\[)([^\]=]+)/g, gatewayUrlTemplateMergeField: /{([^}]+)}/g, lowerCaseThenUpper: /([a-z0-9])([A-Z])/g, upperCaseThenAlpha: /([A-Z])([A-Z][a-z])/g, hasVariable: /\$\{(.*?)\}/g, isFormString: /^\w+=.+&.*$/, isHTML: /<[^>]+>|&[a-zA-Z0-9]+;|&#[0-9]+;|&#x[0-9A-Fa-f]+;/, isJSONObject: /^\s*{.*}$/, isNumeric: /^[0-9\.]+$/, leadingSlash: /^\/+/, nothing: /^(.)/, notAlphaNumeric: /[^a-zA-Z0-9]/, pipeSplitter: /(?<!\|)\|(?!\|)(?![^\[]*\])/, pipeSplitterAndTrim: /\s*\|\s*/, dash: /-/g, xy: /[xy]/g, selectorBranchSplitter: /\s*,\s*(?![^"']*["'][^"']*$)/, selectorSegmentSplitter: /(?<=[^\s>+~|\[])\s+(?![^"']*["'][^"']*$)|\s*(?=\|\||[>+~](?![^\[]*\]))\s*/, spaceSplitter: /\s+/ }), resolveShape: Object.freeze({ startFlags: new Set(['[', '?', '{']), endFlags: Object.freeze({ '.': !0, '!': !1, '-': null, '?': undefined }), closers: Object.freeze({ '{': '}', '[': ']' }) }), unitTypeCollectionNameToUnitTypeMap: Object.freeze({ apis: 'api', components: 'component', content: 'content', context: 'context', facets: 'facet', gateways: 'gateway', hooks: 'hook', interpreters: 'interpreter', languages: 'language', libraries: 'library', ais: 'ai', namespaces: 'namespace', patterns: 'pattern', resolvers: 'resolver', snippets: 'snippet', transforms: 'transform', types: 'type' }), unitTypeMap: Object.freeze({ api: ['apis', 'API'], component: ['components', 'Component'], content: ['content', 'Collection'], context: ['context', Object], facet: ['facets', 'Facet'], gateway: ['gateways', 'Gateway'], hook: ['hooks', Function], interpreter: ['interpreters', Object], language: ['languages', 'Language'], library: ['libraries', Object], ai: ['ais', 'AI'], namespace: ['namespaces', URL], pattern: ['patterns', RegExp], resolver: ['resolvers', Function], snippet: ['snippets', HTMLElement], transform: ['transforms', 'Transform'], type: ['types', 'Type'] }), valueAliases: Object.freeze({ 'null': null, 'undefined': undefined, 'false': !1, 'true': !0, '-': null, '?': undefined, '!': !1, '.': !0 }) }) }, activateTag: {
        value: async function (tag) {
            ...
            for (const subspaceName of (subspaces)) {
                ...
                componentClass.template.innerHTML = template.innerHTML.replace(new RegExp(`<${subspaceName}-`, 'g'), `<${virtualSubspaceName}-`).replace(new RegExp(`</${subspaceName}-`, 'g'), `</${virtualSubspaceName}-`).replace(new RegExp(` is='${subspaceName}-`, 'g'), ` is='${virtualSubspaceName}-`).replace(new RegExp(` is="${subspaceName}-`, 'g'), ` is="${virtualSubspaceName}-`).replace(new RegExp(` is=${subspaceName}-`, 'g'), ` is=${virtualSubspaceName}-`)
        ...
        value: async function (unitTypeCollection, unitTypeCollectionName, packageUrl, packageKey, pkg) {
            ...
            if (unitTypeCollectionName === 'interpreters') {
                ...
                for (const [matcher, interpreter] of unitTypeCollection) {
                    allValid = (matcher instanceof RegExp) && isPlainObject(interpreter) && interpreter.name && (typeof interpreter.name === 'string') && (typeof interpreter.parser === 'function') && (typeof interpreter.handler === 'function') && (!interpreter.binder || (typeof interpreter.binder === 'function'))
                    if (!allValid) break
            ...
            const promises = [], unitType = this.sys.unitTypeCollectionNameToUnitTypeMap[unitTypeCollectionName], [, unitClassName] = this.sys.unitTypeMap[unitType], unitClass = typeof unitClassName === 'string' ? this[unitClassName] : unitClassName
    ...
    }, defaultResolver: { value: async function (unitKey, unitType) { return this.runFragment('defaultresolver', unitKey, unitType) } }, installGateway: {
        value: async function (protocol) {
            ...
            for (let manifest of gatewayManifests) {
                ...
                let connection
                switch (typeof head) {
                    case 'string': if ((await fetch(`${window.location.protocol}//${head}`, { method: 'HEAD' })).ok) connection = !0; break
                ...
                if (auto) {
                    ...
                    this.app._observers.get(useGateway).observe(document.documentElement, { subtree: !0, childList: !0, attributes: !0, attributeFilter: urlAttributes })
                }
        ...
        value: async function (moduleName) {
            ...
            return Object.defineProperty(this.modules, moduleName, { enumerable: !0, value: Object.freeze(Object.defineProperties({}, module)) })[moduleName]
        }
    }, mountElement: {
            ...
            if (customTag) {
                ...
                if (isAttr) {
                    ...
                    if (componentInstance.disconnectedCallback || componentInstance.adoptedCallback || componentInstance.attributeChangedCallback) {
                        const observer = new MutationObserver(mutations => {
                            for (const mutation of mutations) {
                                switch (mutation.type) {
                                    case 'childList': for (const removedNode of (mutation.removedNodes ?? [])) {
                        ...
                        observer.observe(element, { childList: !0, subtree: !1, attributes: !0, attributeOldValue: !0, characterData: !0 })
                ...
                if (root) {
                    ...
                    observer.observe(root, { subtree: !0, childList: !0 })
        ...
        value: async function (facetContainer) {
            ...
            switch (type) {
                case 'directives/element': if (!this.modules.compile) return
                    ...
                    break
                case 'application/element': if (!src || this.app.facets[src]) break
            ...
            const rootNode = facetContainer.getRootNode(), fields = {}, cells = {}, context = Object.freeze(rootNode instanceof ShadowRoot ? { ...this.env.context, ...Object.fromEntries(Object.entries(rootNode.host.dataset)) } : this.env.context)
            ...
            facetInstance.observer.observe(facetContainer, { attributes: !0, attributeFilter: ['disabled'] })
        ...
        value: function (input, flag) {
            const isQs = flag === '?', isArray = flag === '[', delimiter = isQs ? '&' : ',', subDelimiter = isQs ? '=' : ':', nesters = ['"', "'", ...(isQs ? [] : ['[', ']', '{', '}'])], result = isArray ? [] : {}, entries = this.resolveShapeSplitIgnoringNesting(input.slice(1, -1), delimiter, nesters)
            for (const entry of entries) {
                if (isArray) { result.push(this.resolveShape(entry.trim())); continue }
                const [rawKey, rawValue] = this.resolveShapeSplitIgnoringNesting(entry, subDelimiter, nesters, !0)
            ...
            constructor({ api, model, promptTemplates = {} }) {
                ...
                if (model) {
                    switch (typeof model) {
                        ...
                        case 'string': this.modelWrapper = async input => (await (this.model ??= await E.resolveUnit(model, 'model')).run(input))
                            new E.Job(async function () { await E.resolveUnit(model, 'model') }, `model:${model}`)
                            break
                        case 'object': if (model instanceof E.Model) this.modelWrapper = async input => (await (this.model ??= model).run(input))
                ...
                if (api) {
                    switch (typeof api) {
                        ...
                        case 'string': this.apiWrapper = async input => (await (this.api ??= await E.resolveUnit(api, 'api')).run(input))
                            new E.Job(async function () { await E.resolveUnit(api, 'api') }, `api:${api}`)
                            break
                        case 'object': if (api instanceof E.API) this.apiWrapper = async input => (await (this.api ??= api).run(input))
        ...
        enumerable: !0, value: class {
            ...
            constructor({ api = {}, ai = {} }) {
                ...
                switch (typeof api) {
                    case 'function': this.apiWrapper = api.bind(this); break
                    case 'string': const [apiName, apiAction] = api.split(E.sys.regexp.pipeSplitterAndTrim)
                        ...
                        break
                    default: if (E.isPlainObject(api)) {
                ...
                if (ai) {
                    switch (typeof ai) {
                        ...
                        case 'string': const [aiName, aiPrompt] = ai.split(E.sys.regexp.pipeSplitterAndTrim)
                            ...
                            break
                        default: if (E.isPlainObject(ai)) this.aiWrapper = async (prompt) => (await (this.api ??= new E.AI(engine)).run(prompt, undefined, envelope))
                    }
                    if (this.aiWrapper) this.engine = async (slug, envelope) => { return this.aiWrapper(E.resolveVariable(await this.apiWrapper(slug, envelope), envelope, { merge: !0 })) }
                }
        ...
        enumerable: !0, value: class extends HTMLElement {
            static E
            ...
            constructor() {
                ...
                try {
                    if (style || template) {
                        ...
                        if (style) shadowRoot.append(style.cloneNode(!0))
                        if (template) shadowRoot.append(...template.content.cloneNode(!0).children)
                    }
                } catch (e) { }
            }
            ...
            dispatchEvent(event) {
                ...
                let virtualElement = E.app._components.virtualsFromNatives.get(this), nativeElement = E.app._components.nativesFromVirtuals.get(this), eventName = event.type === 'default' ? undefined : event.type
            ...
            async run(container, { fields, cells, context }) {
                ...
                for (const statement of statements) {
                    ...
                    for (const step of steps) {
                        stepIndex++
                        const position = `${statementIndex}-${stepIndex}`, { label, labelMode, defaultExpression, signature } = step, { interpreter: interpreterKey, variables } = signature, descriptor = { ...(signature.descriptor ?? {}) }, { signal } = descriptor, envelope = { descriptor, labels, fields, cells, context, variables }
                        ...
                        container.addEventListener(stepIndex ? `done-${statementIndex}-${previousStepIndex}` : 'run', async () => {
                            if (disabled) return
                            const handlerEnvelope = { ...envelope, fields: Object.freeze(await E.flatten(fields)), cells: Object.freeze(await E.flatten(cells)), labels: Object.freeze({ ...labels }) }, value = previousStepIndex !== undefined ? labels[`${previousStepIndex}`] : undefined, detail = await handler(container, position, handlerEnvelope, value) ?? (defaultExpression ? E.resolveVariable(defaultExpression, { ...handlerEnvelope, value }) : undefined)
                            if (detail !== undefined) container.dispatchEvent(new CustomEvent(`done-${position}`, { detail }))
        ...
        enumerable: !0, value: class {
            ...
            constructor({ defaultTokenValue = '', tokens = {}, virtual = {}, envelope }) {
                ...
                switch (typeof virtual.engine) {
                    case 'string': break
                    case 'object': const validEngine = !1
                ...
                if (!(Array.isArray(virtual.preload) || virtual.preload === !0)) delete virtual.preload
                if (typeof virtual.base !== 'string') delete virtual.base
                if (!(typeof defaultTokenValue === 'string' || defaultTokenValue === !0)) defaultTokenValue = ''
                Object.assign(this, { defaultTokenValue, tokens: Object.freeze(tokens), virtual })
                if (typeof virtual.engine === 'string') {
                    const [engineType, engineNamePlusIntent] = virtual.engine.trim().split(E.sys.regexp.colonSplitter), [engineName, engineIntent] = engineNamePlusIntent.split(E.sys.regexp.pipeSplitter), loadEngineJob = new E.Job(async function () { await await E.resolveUnit(engineName, engineType) }, `${engineType}:${engineName}`)
                    virtual.engine = loadEngineJob.complete.then(async () => {
                        const engine = await E.resolveUnit(engineName, engineType), validEngine = !1
                        ...
                        if (engineIntent != null) virtual.engineIntent = engineIntent || !0
                        virtual.lang ??= {}
        ...
        enumerable: !0, value: class {
            ...
            constructor({ engine, matches = {}, mode, scopeSelector, name, namespace, labels = {}, defaultValue = '', envelope }) {
                ...
                name = (name && typeof name === 'string') ? name.replaceAll(E.sys.regexp.notAlphaNumeric, '').toLowerCase() : E.generateUuid(!0)
                ...
                if (mode) {
                    ...
                    switch (mode) {
                        case 'audio': for (const a of audioVideoAttrs) this.matches[`@${a}`] ??= `audio[data-${namespace}-attr-${a}][${a}]`
                            break
                        case 'css': this.matches.textContent ??= `style[data-${namespace}-text-content]`
                        ...
                        case 'image': for (const a of ['alt', 'formaction', 'formenctype', 'formmethod', 'formnovalidate', 'formtarget', 'height', 'src', 'width']) this.matches[`@${a}`] ??= `img[data-${namespace}-attr-${a}][${a}]`
                            this.matches['@poster'] ??= `video[data-${namespace}-attr-poster][poster]`
                            break
                        case 'js': this.matches.textContent ??= `script[type="application/javascript"][data-${namespace}-text-content]`
                        ...
                        case 'video': for (const a of [...audioVideoAttrs, 'disablepictureinpicture', 'height', 'playsinline', 'poster', 'width']) this.matches[`@${a}`] ??= `video[data-${namespace}-attr-${a}][${a}]`
                            break
                        case 'lang': attributes.add('lang')
                        case 'text': this.matches.textContent ??= `[data-${namespace}-text-content]`
            ...
            support(node) {
                ...
                observer.observe(node, { subtree: !0, childList: !0, attributeFilter: this.attributeFilter })
            ...
            constructor(stepChain, pipelineState = {}) {
                ...
                for (let [stepKey, stepValue] of stepChain.entries()) {
                    ...
                    if (typeof stepKey !== 'string') stepKey = `${stepKey}`
                        ...
                        this.steps.set(stepKey, async (input, envelope) => {
                            const jsonata = (E.app.libraries.jsonata ??= await E.resolveUnit('jsonata', 'library')), expression = this.stepIntermediates.get(stepKey) ?? this.stepIntermediates.set(stepKey, jsonata(`(${step.trim()})`)).get(stepKey)
                            return expression.evaluate(input, { envelope })
                        })
                    } else if (this.isPlainObject(stepValue) && (Object.keys(stepValue).length === 1)) {
                        const { unitType, unitNamePlusIntent } = stepValue.entries()[0], [unitName, unitIntent] = unitNamePlusIntent.split(E.sys.regexp.pipeSplitter), stepClassName = (E.sys.unitTypeMap[unitType] ?? [])[1]
        ...
        enumerable: !0, value: class {
            ...
            constructor(typeDefinition, typeName) {
                ...
                switch (typeof typeDefinition) {
                    ...
                    case 'object': if (typeDefinition instanceof E.Validator) {
                        ...
                        break
                    case 'string': this.engine = async (input, verbose, envelope) => {
                        const xdr = await E.resolveUnit('xdr', 'library')
...
Object.defineProperties(ElementHTML, {
    Cell: {
        enumerable: !0, value: class extends ElementHTML.State {
            constructor(name, initialValue) {
...
for (const k in ElementHTML.env) Object.defineProperty(app, k, { configurable: !1, enumerable: !0, writable: !1, value: {} })
for (const className of ['API', 'Collection', 'Component', 'Facet', 'Gateway', 'Job', 'Language', 'Transform', 'Type', 'Validator']) Object.defineProperty(ElementHTML[className], 'E', { configurable: !1, writable: !1, value: ElementHTML })
...
if (initializationParameters.has('packages')) {
    ...
    if (importmapElement) try { imports = Object.assign(importmap, JSON.parse(importmapElement.textContent.trim())).imports } catch (e) { } else if (initializationParameters.get('packages')) for (const p of packageList) imports[p] = `./packages/${p}.js`
    else {
